...
export async function createRecord({ ownerId, videoId, driveId, reporterId, metadata, batchId, date }) {
  await connectDB();
  ...
  const record = await ArchiveRecord.create({
    ...
    metadata, ownerId, batchId, archiveDate: date, status: 'committed',
  });
...
export async function updateRecord({ id, ownerId, ...updates }) {
  ...
  if (updates.date) {
    allowed.archiveDate = new Date(updates.date);
}
  if (updates.driveId) {
