...
export default function BatchesPage() {
  ...
  const statusVariant = (s) =>
    ({ open: "info", committed: "success", archived: "purple" })[s] ||
    "default";
    ...
    <div className="space-y-4 animate-fadeIn">
        ...
        <div className="space-y-2">
          {batches.length === 0 ? (
            <div className="card p-12 text-center text-textMuted">
              No batches found. Add records to create your first batch.
            </div>
          ) : (
            batches.map((b) => (
              <div
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button
                      onClick={() => handleExport(b._id, "csv")}
                      className="p-1.5 border border-inputBorder rounded hover:bg-cardHover text-textMuted hover:text-primary transition-colors"
                      title="Export CSV"
                    >
                      <Download size={14} />
                    </button>
                    ...
                    <button
                      onClick={() => handleExport(b._id, "json")}
                      className="p-1.5 border border-inputBorder rounded hover:bg-cardHover text-textMuted hover:text-primary transition-colors"
                      title="Export JSON"
                    >
                      <Archive size={14} />
                    ...
                    <button
                      ...
                      className="p-1.5 rounded bg-danger/30 hover:bg-danger/40 text-danger  transition-colors"
                    >
                      <Trash2 size={14} />
                    </button>
                  </div>
      ...
      >
        <p className="text-sm text-textMuted mb-5">
          Once committed, records will be locked. The batch status changes to
          "committed".
        </p>
        <div className="flex gap-2 justify-end">
          <Button size="lg" variant="outline" onClick={() => setCommitId(null)}>
            Cancel
          </Button>
          <Button size="lg" loading={acting} onClick={handleCommit}>
            <CheckCircle size={14} />
            Commit Batch
          </Button>
        </div>
      ...
      >
        <p className="text-sm text-textMuted mb-5">
          This will soft-delete the batch and all its records. Recoverable by
          admin.
        </p>
        <div className="flex gap-2 justify-end">
          <Button size="lg" variant="outline" onClick={() => setDelId(null)}>
            Cancel
          </Button>
          <Button
            size="lg"
