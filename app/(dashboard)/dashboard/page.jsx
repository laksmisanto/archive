...
function StatCard({ icon: Icon, label, value, sub, color = "blue", loading }) {
  ...
  return (
    <div className="bg-dashboardCardBg border border-inputBorder rounded-lg p-5">
      ...
      <div className="mt-3">
        {loading ? (
          <div className="h-8 w-20 bg-dashboardCardBg rounded animate-pulse" />
        ) : (
...
export default function DashboardPage() {
  ...
  const lastBatchDate = stats?.lastBatch?.date
    ? new Date(stats.lastBatch.date).toLocaleDateString("en-US", {
        weekday: "short",
        month: "short",
        day: "numeric",
      })
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {stats?.showWarning && (
        <Alert
          type="warning"
          title="Today's archive data has not been uploaded."
        >
          It is past 6:00 PM and no records have been added to today's batch.
          Please upload your daily archive data.
        </Alert>
      )}
            ...
            <div>
              {todayBatchStatus ? (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-textPrimary capitalize font-medium">
                    {todayBatchStatus}
                  </span>
                  <span className="text-xs text-textMuted">
                    — {stats?.todayBatch?.recordCount || 0} records
                  </span>
                </div>
              ) : (
              ...
              <div className="mt-3 flex gap-2">
                <Link href="/records/new">
                  <Button size="lg">Add Record</Button>
                </Link>
                <Link href="/batches">
            ...
            <div>
              ...
              {stats?.lastBatch && (
                <p className="text-xs text-textMuted mt-1">
                  {stats.lastBatch.recordCount} records in that batch
                </p>
              )}
            </div>
          )}
