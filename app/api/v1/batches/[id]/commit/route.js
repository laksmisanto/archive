...
export async function POST(request, { params }) {
  return withAuth(request, async (req) => {
    try {
      await connectDB();
      const batch = await Batch.findOne({ _id: params.id, ownerId: req.user.id, status: 'open', deletedAt: null });
      if (!batch) return notFound('Open batch not found');
      batch.status = 'committed';
      batch.committedAt = new Date();
      await batch.save();
      await ArchiveRecord.updateMany({ batchId: params.id, ownerId: req.user.id, status: 'draft', deletedAt: null }, { status: 'committed' });
