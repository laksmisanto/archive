...
export default function ReportersAdminPage() {
  ...
  const load = () => {
    setLoading(true);
    fetch('/api/v1/reporters').then(r => r.json()).then(d => {
    ...
    <div className="space-y-4 animate-fadeIn">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => setAddModal(true)}><Plus size={14} />Add Reporter</Button>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="py-12 text-center text-textMuted text-sm">No reporters yet. Add your first reporter.</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-divider bg-surface">
                <th className="text-left px-4 py-3 text-xs font-semibold text-textMuted uppercase tracking-wider">Name</th>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center flex-shrink-0">
                        <UserCheck size={13} className="text-purple-600 dark:text-purple-400" />
                      </div>
                      <span className="font-medium text-textPrimary">{r.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3"><Badge variant={r.isActive ? 'success' : 'danger'}>{r.isActive ? 'Active' : 'Inactive'}</Badge></td>
                  <td className="px-4 py-3 text-xs text-textMuted hidden sm:table-cell">{new Date(r.createdAt).toLocaleDateString()}</td>
                  <td className="px-4 py-3">
      ...
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Add Reporter" size="sm">
        <form onSubmit={handleAdd} className="space-y-4">
          <Input label="Reporter Name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Shanto, Rahim" required autoFocus />
          <div className="flex gap-2 justify-end pt-1">
            <Button type="button" variant="outline" onClick={() => setAddModal(false)}>Cancel</Button>
            <Button type="submit" loading={saving}>Add Reporter</Button>
          </div>
        </form>
      </Modal>

      <Modal open={!!delId} onClose={() => setDelId(null)} title="Remove Reporter" size="sm">
        <p className="text-sm text-textMuted mb-5">This reporter will be soft-deleted. Existing records are not affected.</p>
